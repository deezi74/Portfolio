...
// The original used __android_log_is_loggable(), which also checks the
// "adb shell setprop log.tag.<TAG>" runtime override - but that API needs
// Android 30+, and Luna's minSdk is 26. A plain compile-time level compare
// gives up that one runtime override but keeps everything else working.
